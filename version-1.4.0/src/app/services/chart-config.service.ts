...
export class ChartConfigService {
  ...
  /**
   * Generate chart data from column analysis
   * Returns labels and data arrays for chart rendering
   */
  generateChartData(
    columnName: string,
    ...
    if (columnType === 'Text') {
      ...
      const sorted: [string, number][] = Object.entries(valueCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, maxItems);
    ...
    if (columnType === 'Numeric') {
      const numValues = values.map(v => parseFloat(v)).filter(v => !isNaN(v));
      if (numValues.length === 0) return { labels: [], data: [] };

      const min = Math.min(...numValues);
      const max = Math.max(...numValues);
      const binCount = Math.min(maxItems, 10);
      const binSize = (max - min) / binCount || 1;
      ...
      for (let i = 0; i < binCount; i++) {
        const binStart = min + i * binSize;
        const binEnd = binStart + binSize;
        const count = numValues.filter(v => 
  ...
  createChartDataset(
    data: number[],
    label: string,
    chartType: 'bar' | 'pie' | 'line' | 'doughnut' = 'bar'
  ): ChartConfiguration['data']['datasets'][0] {
    const colors = this.powerBIColors;
    const borderColors = this.powerBIBorderColors;
    ...
    if (chartType === 'bar') {
      return { ...baseDataset, borderRadius: 6 };
    }
    if (chartType === 'pie' || chartType === 'doughnut') {
      return { ...baseDataset, borderWidth: 1 };
    }
    return { ...baseDataset, fill: true, tension: 0.4 };
  }
}
